...
void handle_moves(game g) {
    ...
    // when the user bar moves, the computer does too, depending on the ball's position
    if (g.ball.y < SCREEN_HEIGHT/2) {
        move_bar(g.computer.v_bar, -1);
    } else {
        move_bar(g.computer.v_bar, 1);
    }
